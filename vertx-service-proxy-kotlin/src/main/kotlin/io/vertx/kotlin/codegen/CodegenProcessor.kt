...
//                log.warning("Please use 'codegen.generators' option instead of 'codeGenerators' option")
    ...
    private  fun loadGenerators(): List<CodeGenerator> {
        ...
        try {
            descriptors = CodegenProcessor::class.java.classLoader.getResources("codegen.json")
                ...
                try {
                    ...
                    if (uri.endsWith(".kt")) {
                        if (processing.model is ProxyModel) {
                            val data = (processing.model.element.kotlinMetadata as KotlinClassMetadata?)!!.data
                            data.classProto.functionList.forEach {f->
                                val functionName = data.nameResolver.getString(f.name)
//                                println("$functionName/${it.returnType.hasTypeAliasName()}/${it.returnType.hasClassName()}/${it.returnType.hasNullable()}")
//                                println(data.nameResolver.getString(it.returnType.typeAliasName))
//                                println(data.nameResolver.getString(it.returnType.className))
                                println(functionName + "====" + f.valueParameterCount + "===")
                                f.valueParameterList.forEach {
                                    f
                                    val type = data.nameResolver.getClassId(it.typeId)
                                    println("===${it.type}")
                                }
                            }
//                            val functionMap = data.classProto.functionList.associateBy { it.name }
                            val kotlinMethods = ArrayList<KotlinMethodInfo>()
                            val kotlinImport = HashSet<String>()

                            var relativeName: String? = MVEL.executeExpression(processing.generator.filenameExpr, HashMap<String, Any>().apply { put("ifaceFQCN", processing.model.fqn) }) as String?
                            if (!relativeName!!.startsWith(processing.model.fqn)) {
                                kotlinImport.add(processing.model.fqn)
                            }
                            ...
                            if (field != null) {
                                ...
                                methods.mapTo(kotlinMethods) { (element, info)  ->
                                    KotlinMethodInfo(info as ProxyMethodInfo, element, ::println)
                                }
